...
def test_wind_battery_hydrogen_optimize(input_params):
    design_res = wind_battery_hydrogen_optimize(int(8760/12), input_params, verbose=False, plot=True)
    assert design_res['wind_mw'] == pytest.approx(811.57, rel=1e-2)
    assert design_res['batt_mw'] == pytest.approx(87.915, rel=1e-2)
    assert design_res['batt_mwh'] == pytest.approx(626.55, rel=1e-2)
    ...
    assert design_res['annual_rev_E'] == pytest.approx(-24630099, rel=1e-2)
    assert design_res['NPV'] == pytest.approx(-562400493, rel=1e-2)


def test_wind_battery_hydrogen_optimize_cheap_hydrogen(input_params):
    input_params["turbine_cap_cost"] *= 0.1
    design_res = wind_battery_hydrogen_optimize(int(8760/12), input_params, verbose=False, plot=False)
    assert design_res['wind_mw'] == pytest.approx(811.57, rel=1e-2)
    assert design_res['batt_mw'] == pytest.approx(54.418, rel=1e-2)
    assert design_res['batt_mwh'] == pytest.approx(522.859, rel=1e-2)
    assert design_res['pem_mw'] == pytest.approx(1.58, abs=1)
    assert design_res['tank_tonH2'] == pytest.approx(5.229, abs=3)
    assert design_res['turb_mw'] == pytest.approx(33.478, abs=3)
    assert design_res['annual_rev_h2'] == pytest.approx(0, abs=1)
    assert design_res['annual_rev_E'] == pytest.approx(-26075695, rel=1e-2)
    assert design_res['NPV'] == pytest.approx(-560605858, rel=1e-2)
